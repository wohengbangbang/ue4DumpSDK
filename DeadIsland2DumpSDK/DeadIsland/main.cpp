...
std::string GetType(uint64_t Object) { 
    ...
    if (typeName == "EnumProperty") {
        // 0x78 处存储的是 UEnum 指针
        //uint64_t EnumPtr = ReadMemory<uint64_t>(Object + 0x78);
        //return GetNameForObject(EnumPtr);
        return typeName = "enum " + typeName;
    }
    ...
    if (typeName == "ByteProperty") {
        ...
        //uint64_t EnumPtr = ReadMemory<uint64_t>(Object + 0x78);
        //if (EnumPtr) return GetNameForObject(EnumPtr);
        return "uint8_t";
    }
...
// 拼接 DumpClass 并写入到文件
std::string DumpClass(uint64_t Object) {
    // 第一个类成员的位置 + 0x50
    // 第二个成员的位置   + 0x20 + 0x20 ...
    // 第二个成员的名称   + 0x28
    // 大小             + 0x3c
...
// dump func
void DumpClassFunction(uint64_t Object) {
    // 
}
    ...
    for (size_t i = 0; i < nowItmeNum; i++) {
        ...
        while (sObj) {
            ...
            if (GetNameForObject(sObj) == "Class") {
                // 开始进行dump class 
                dumpToFile(DumpClass(Object), OUTPUT_CLASS_FILE);
            }
